...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ultroid_bot(
                CreateChannelRequest(
                    title="My Ultroid-X Logs",
                    about="My Ultroid-X Log Group\n\n Join @TeamUltroid | @AlphaXProject",
                    megagroup=True,
                ),
            )
...
async def customize():
    ...
    try:
        ...
        await ultroid_bot.send_message(
            "botfather",
            f"✨ Powerful Ultroid-X Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @TeamUltroid | @AlphaXProject ✨",
        )
        await asyncio.sleep(2)
...
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
    ...
    try:
        await ultroid_bot(JoinChannelRequest("AlphaXProject"))
    except Exception as er:
        LOGS.exception(er)
